...
BASE_OUTPUT_DIR = "trained_models_official_w_bn_newdata4"
...
def get_output(path,file_name,normalized = True):
    ...
    label = cv2.resize(label,(150,150))
    label = label.reshape(-1)
    label = label.astype(np.float32)
    if normalized:
        normalizedImg = np.zeros_like(label)
        normalizedImg = cv2.normalize(label,  normalizedImg, 0, 1, cv2.NORM_MINMAX)
        label1 = normalizedImg.reshape(-1)
    return label1
...
def image_generator(img_file,label_file, batch_size = 64):
    while True:
        ...
        #print(np.where((batch_x[0]>0)&(batch_x[0]<0.99)))
...
if __name__ == '__main__':
    ...
    x = Conv2DTranspose( classes , kernel_size=(16,16) ,  strides=(8,8) , use_bias=False,kernel_regularizer=l2(weight_decay) )(x)
    ...
    validation_generator = image_generator("train_aug","train_aug_GT",BATCH_SIZE)  
    callbacks_list = [
            ModelCheckpoint(
    ...
    model.compile(optimizer=optimizer,
                    loss=binary_crossentropy_with_logits)#tf.losses.sigmoid_cross_entropy)
    
    model.fit_generator(train_generator,
                              steps_per_epoch=int((117486//BATCH_SIZE)*0.8),#1014545//options.batch_size,#int((1014545//options.batch_size)*0.8), #1014545//options.batch_size,#100//options.batch_size
                              epochs=NUM_EPOCHS,
                              validation_data=validation_generator,
                              validation_steps=int((117486//BATCH_SIZE)*0.2),#98975//options.batch_size,#int((1014545//options.batch_size)*0.2),#98975//options.batch_size,#10//options.batch_size
